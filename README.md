# 🇨🇦 Canadian Stock Market Telegram Monitor Bot

A serverless, automated Telegram bot that monitors Canadian stock exchanges (TSX & TSXV), filters for liquidity and volume spikes, identifies high-precision price anomalies, generates professional auditor-style financial audits via the **Groq API** (Llama 3.3), and sends reports hourly.
...
3. **Data Retrieval**: High-precision intraday and historical data are fetched via `yfinance` for TSX/TSXV tickers.
...
5. **Core Filtering Logic**:
   - **Liquidity Filter**: Discards any stock with a 20-period average volume under 1,000,000.
   - **Volume Spike**: Identifies stocks where current volume is $\ge 3\%$ above the 20-day moving average.
   - **Anomalies**: Spots stocks with extreme price moves ($> 5\%$ change) without a volume spike.
6. **AI Commentary**: Every 1 hour (4 triggers), the bot feeds the data to the **Groq API** (`llama-3.3-70b-versatile`) representing a professional financial auditor, producing buy/sell advice and price speculation.
7. **Telegram Broadcast**: The report is formatted in clean, monospaced HTML and sent to your Telegram channel/chat.
...
3. Select the **`gist`** scope and (optionally) **`repo`** scope (if needed to trigger dispatch, though `repo` or `workflow` is required to call repository dispatch).
...
2. Create a new Cron Job with the following details:
   - **Title**: `Canadian Stock Bot Trigger`
...
The bot has a seeded file `data/tickers.json` with highly liquid Canadian stocks. To dynamically expand or refresh this list from Wikipedia or `cad_tickers`, run:
```bash
python src/fetch_tickers.py
```
This will rebuild `data/tickers.json` with all active TSX and TSXV tickers.
